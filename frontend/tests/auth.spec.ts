...
test("auth: invalid credentials stay on login and show error", async ({
  page,
}) => {
  ...
  const clickLogin = async () => {
    ...
    } else if (await fallbackBtn.isVisible().catch(() => false)) {
  ...
  // Error may appear as toast or inline text; check common locations
  const toast = page
    .getByText(
      /invalid username or password|invalid email or password|login failed/i,
    )
    .first();
  if (await toast.isVisible().catch(() => false)) {
    await expect(toast).toBeVisible();
  } else {
    const inline = page.locator("text=/invalid|password/i").first();
...
test("auth: direct protected route access redirects to login", async ({
  page,
}) => {
  await page.goto("/app");
  await expect(page).toHaveURL(/\/login/);
  await expect(
    page.getByRole("heading", { name: /Asset Management by Krubles Login/i }),
  ).toBeVisible();
});
...
test("auth: silent refresh on 401 triggers retry and succeeds", async ({
  page,
}) => {
  await page.route("**/actuator/health", async (route) => {
    await route.fulfill({
  ...
  await page.evaluate(
    () =>
      window.__e2eLogin && window.__e2eLogin("initialAccess", "initialRefresh"),
  );
...
test("auth: logout returns to login", async ({ page }) => {
  ...
  await page.goto("/login?e2e=1");
  await page.evaluate(
    () => window.__e2eLogin && window.__e2eLogin("testAccess", "testRefresh"),
  );
  await page.goto("/app");
...
test.skip("auth: session expiring banner refreshes and hides", async ({
  page,
}) => {
  await page.route("**/actuator/health", async (route) => {
    await route.fulfill({
  ...
  await page.goto("/login?e2e=1");
  await page.evaluate(
    () => window.__e2eLogin && window.__e2eLogin("shortAccess", "shortRefresh"),
  );
  await page.goto("/app");
  ...
  await page.waitForFunction(
    () =>
      !!document.body.textContent?.match(/Your session is about to expire/i),
    null,
    { timeout: 10000 },
  );
