...
declare global {
  interface Window {
    __HAS_GLOBAL_ERROR_HANDLERS__?: boolean;
  }
}
...
(() => {
  try {
    ...
    const urlFlag =
      ...
      window.location &&
      /[?&](e2e|e2eAuto)=1/.test(window.location.search);
    const envFlag =
      typeof window !== "undefined" &&
    ...
    if (isPlaywright || urlFlag || envFlag || persisted) {
      ...
      (window as any).__e2eLogin = (
        ...
        try {
          ...
          // Do not auto-navigate here; tests will navigate explicitly after seeding auth
        } catch {}
      };
