...
const GoogleLoginButton = ({ onError, onLoading, disabled }) => {
  ...
  useEffect(() => {
    // Don't load Google services if no valid client ID is configured
    if (!GOOGLE_CLIENT_ID || GOOGLE_CLIENT_ID === 'your-google-client-id-here' || GOOGLE_CLIENT_ID === 'demo-google-client-id') {
  ...
  if (!GOOGLE_CLIENT_ID || GOOGLE_CLIENT_ID === 'your-google-client-id-here' || GOOGLE_CLIENT_ID === 'demo-google-client-id') {
    return (
      ...
      >
        ...
        <span className="text-gray-500 font-medium">Sign in with Google (Not Configured)</span>
      </button>
    );
  }
