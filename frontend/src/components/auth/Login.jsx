...
export const Login = () => {
  ...
  const performHealthCheck = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (isDev)
          console.debug(
            "[HealthCheck] /healthz failed, falling back to /actuator/health:",
      ...
      if (status === "UP") {
        ...
        setBackendStatus({
          state: latency < 500 ? "up" : "warn",
          message: latency < 500 ? "Online" : "Stable",
        });
      ...
      if (isDev)
        console.warn(
          "[HealthCheck] primary request failed:",
      ...
      if (respData && typeof respData === "object") {
        try {
          ...
          if (mailOnly) {
            if (isDev)
              console.debug(
                "[HealthCheck] 503 with only mail DOWN -> degraded",
              );
            lastHealthSuccessRef.current = Date.now();
            setBackendStatus({ state: "warn", message: "Degraded" });
            return;
      ...
      const shouldFallback =
        !lastHealthSuccessRef.current ||
        Date.now() - lastHealthSuccessRef.current > 15000;
      if (shouldFallback) {
        try {
          // Attempt direct /healthz first, then actuator if needed
          let direct = await fetch("/api/healthz", {
            headers: { Accept: "application/json" },
          });
          if (!direct.ok) {
            direct = await fetch("/api/actuator/health", {
              headers: { Accept: "application/json" },
            });
          }
          if (direct.ok) {
            ...
            if (json.status === "DOWN") {
              if (isDev)
                console.debug(
                  "[HealthCheck] fallback reachable but DOWN -> marking degraded",
                );
              setBackendStatus({ state: "warn", message: "Degraded" });
              return;
            }
          ...
          /* swallow direct fallback errors */
        }
      }
      setBackendStatus({ state: "down", message: "Offline" });
    }
  ...
  useEffect(() => {
    ...
    const wrapped = async () => {
      if (!cancelled) await performHealthCheck();
    };
    wrapped(); // initial
    ...
    return (
      <FirstLoginPasswordChange
        ...
        onSuccess={() => {
          ...
          setFormError("Password changed. Please log in.");
        }}
      />
    );
  }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="login-form" noValidate>
        ...
        <div className="login-header">
          <h2 id="login-heading" className="login-title">
            ...
            >
              ...
              />
              <path
                d="M9 7V6a3 3 0 0 1 3-3v0a3 3 0 0 1 3 3v1"
                stroke="currentColor"
                strokeWidth="1.8"
                strokeLinecap="round"
              />
        ...
        <fieldset style={{ border: "none", padding: 0, margin: 0 }}>
          ...
          <div className="form-group">
            ...
            <input
              ...
              disabled={
                isSubmitting || (backendStatus.state === "down" && !isE2E)
              }
              {...register("username")}
            />
            {errors.username && (
        ...
        <button
          ...
          disabled={
            ...
            googleLoading ||
            microsoftLoading ||
            (backendStatus.state === "down" && !isE2E)
          }
        ...
        >
          If your account has MFA enabled, you’ll enter a 6‑digit code after
          signing in.
        </div>
